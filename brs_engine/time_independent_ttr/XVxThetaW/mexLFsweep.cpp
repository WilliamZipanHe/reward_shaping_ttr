...
//             c = (sigma1[i][j][k]/dx[0] + sigma2[i][j][k]/dx[1] + sigma3/dx[2]);
            ...
            if (k == 0)
            {
                ...
                r = (phi[i][j][k+1][h] - phi[i][j][N3-1][h])/(2*dx[2]);
                t = (phi[i][j][k][h+1] - phi[i][j][k][h-1])/(2*dx[3]);
                
                if (q * sin(xs[i][j][k][h][2])/m + t * L / Iyy >= 0)
                {
                    T1 = T1Min;
                }
                else
                ...
                sigma2 =  abs(-transDrag*xs[i][j][k][h][1]/m + T1*sin(xs[i][j][k][h][2])/m + T2*sin(xs[i][j][k][h][2])/m);
                ...
                H = (-1) * ((xs[i][j][k][h][1]) * p \
                        + (-transDrag*xs[i][j][k][h][1]/m + T1*sin(xs[i][j][k][h][2])/m + T2*sin(xs[i][j][k][h][2])/m) * q\
                        + (xs[i][j][k][h][3]) * r\
