...
class TRPO(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, timesteps_per_batch=1024, max_kl=0.01, cg_iters=10, lam=0.98,
                 ...
                 _init_setup_model=True, policy_kwargs=None, full_tensorboard_log=False, **kwargs):
    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=100, tb_log_name="TRPO",
        ...
        # AMEND: logging freqency: every N iters
        logging_freq = 30
        trpo_rewards = []
        trpo_suc_percents = []
        eval_trpo_suc_percents = []
        eval_trpo_rewards = []
        episode_num = 0
        suc_num = 0
            ...
            with self.sess.as_default():
                ...
                while True:
                    ...
                    if self.verbose >= 1 and self.rank == 0:
                        logger.dump_tabular()
                    # ---------------------------------------------------------------------------------
                    # ----------------- plot statistics every logging_freq iters ----------------------
                    episode_num += len(seg['ep_lens'])
                    suc_num += Counter(seg['suc'])[True]
                    trpo_rewards.append(np.mean(reward_buffer))
                    if iters_so_far % logging_freq == 0:
                        trpo_suc_percents.append(suc_num/episode_num)
                        suc_num = 0
                        episode_num = 0
                        # ----------------------------- Evaluate for 5 iters ----------------------------
                        logger.log("start evaluating current policy")
                        eval_pi = self.policy_pi
                        eval_seg_gen = traj_segment_generator(eval_pi, self.env, self.timesteps_per_batch // 2,
                                                            reward_giver=self.reward_giver, gail=self.using_gail, stochastic=False)
                        ...
                        eval_ep_mean_rews = list()
                        eval_ep_mean_lens = list()
                        ...
                        while True:
                            if eval_max_iters and eval_iters_so_far >= eval_max_iters:
                                break
                            logger.log("********** Iteration %i ************" % eval_iters_so_far)

                            eval_seg = eval_seg_gen.__next__()
                            # logger.log("eval_seg:",eval_seg)
                            # added by xlv for computing success percentage

                            eval_suc_num += Counter(eval_seg['suc'])[True]
                            eval_episode_num  += len(eval_seg['ep_lens'])
                            ...
                            eval_ep_mean_lens.append(np.mean(eval_lenbuffer))
                            eval_ep_mean_rews.append(np.mean(eval_rewbuffer))
                            ...
                            logger.record_tabular("TimeElapsed", time.time() - eval_tstart)
                            logger.record_tabular("success percentage", eval_suc_num * 1.0 / eval_episode_num)
                            if MPI.COMM_WORLD.Get_rank() == 0:
                                logger.dump_tabular()
                            eval_trpo_rewards.append(np.mean(eval_rewbuffer))
                        # --------------------------------------------------------------------------
                        eval_trpo_suc_percents.append(eval_suc_num * 1.0 / eval_episode_num)
                        # -------------------------- plotting and saving ----------------------------
                        self.save(self.kwargs['MODEL_DIR'] + '/iter_' + str(iters_so_far // logging_freq))

                        plot_performance(range(len(trpo_rewards)), trpo_rewards,
                                         ylabel=r'avg reward per TRPO learning step',
                                         xlabel='trpo iteration',
                                         figfile=os.path.join(self.kwargs['FIGURE_DIR'], 'trpo_reward'), title='TRAIN')
                        plot_performance(range(len(trpo_suc_percents)), trpo_suc_percents,
                                         ylabel=r'overall success percentage per algorithm step under TRPO',
                                         xlabel='algorithm iteration',
                                         figfile=os.path.join(self.kwargs['FIGURE_DIR'], 'success_percent'),
                                         title="TRAIN")

                        plot_performance(range(len(eval_trpo_rewards)), eval_trpo_rewards,
                                         ylabel=r'avg reward per TRPO eval step',
                                         xlabel='trpo iteration',
                                         figfile=os.path.join(self.kwargs['FIGURE_DIR'], 'eval_trpo_reward'),
                                         title='EVAL')
                        plot_performance(range(len(eval_trpo_suc_percents)), eval_trpo_suc_percents,
                                         ylabel=r'overall eval success percentage per algorithm step under TRPO',
                                         xlabel='algorithm iteration',
                                         figfile=os.path.join(self.kwargs['FIGURE_DIR'], 'eval_success_percent'),
                                         title="EVAL")

                        # save data which is accumulated UNTIL iter i
                        with open(self.kwargs['RESULT_DIR'] + '/trpo_reward_' + 'iter_' + str(
                                iters_so_far // logging_freq) + '.pickle', 'wb') as f2:
                            pickle.dump(trpo_rewards, f2)
                        with open(self.kwargs['RESULT_DIR'] + '/success_percent_' + 'iter_' + str(
                                iters_so_far // logging_freq) + '.pickle', 'wb') as fs:
                            pickle.dump(trpo_suc_percents, fs)

                        # save evaluation data accumulated until iter i
                        with open(self.kwargs['RESULT_DIR'] + '/eval_trpo_reward_' + 'iter_' + str(
                                iters_so_far // logging_freq) + '.pickle', 'wb') as f_er:
                            pickle.dump(eval_trpo_rewards, f_er)
                        with open(self.kwargs['RESULT_DIR'] + '/eval_success_percent_' + 'iter_' + str(
                                iters_so_far // logging_freq) + '.pickle', 'wb') as f_es:
                            pickle.dump(eval_trpo_suc_percents, f_es)
                        # -------------------------------------------------------------------
