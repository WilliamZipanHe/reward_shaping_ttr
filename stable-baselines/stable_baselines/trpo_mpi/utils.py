...
def traj_segment_generator(policy, env, horizon, reward_giver=None, gail=False, stochastic=True):
    """
    ...
    # AMEND: define success percentage. Added by XLV
    suc = False
    ...
    while True:
        if stochastic:
            ...
            action, vpred, states, _ = policy.step(observation.reshape(-1, *observation.shape), states, done, deterministic=True)
        ...
        if step > 0 and step % horizon == 0:
            ...
            if stochastic:
                ...
                _, vpred, _, _ = policy.step(observation.reshape(-1, *observation.shape), deterministic=True)
        ...
        # AMEND: comment the action clipping. Added by XLV
        ...
        if gail:
            ...
            # AMEND: being compatible with our env. BY XLV
            # observation, reward, done, info = env.step(clipped_action[0])
            observation, reward, done, suc, info = env.step(clipped_action[0])
            true_reward = reward
        rewards[i] = reward
